...
interface PluginConfigDataGen extends PluginConfigData {
  fileUrl: string;
  count: number;
}
...
export class DataPlugin extends PluginDataBase {
  ...
  public readonly config: PluginConfigDataGen;

  constructor(config: PluginConfigDataGen) {
    super();
    this.config = config;
  }
  ...
  public async fetchData(): Promise<{ zigs: Zig[]; zags: Zag[] }> {
    ...
    try {
      const zhaDevices = (await (
        await fetch(this.config.fileUrl)

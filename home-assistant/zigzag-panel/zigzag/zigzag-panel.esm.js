...
class ZigzagWC extends LitElement {
    ...
    render() {
      ...
      <div
        class="zigzag"
        @zigClicked=${(event) => this._onZigClicked(event)}
      ></div>
      <div class="buttonBox">
      ...
      <div id="zigViewer">
        <wl-button
          ...
          inverted
          @click=${() => {
            const _zigViewer = this.shadowRoot?.querySelector(`#zigViewer`);
            if (_zigViewer) {
    ...
    _onZigClicked(event) {
        const _zigViewer = this.shadowRoot?.querySelector(`#zigViewer`);
        this._jsonViewer = new c(event.detail.zig, 1, {
